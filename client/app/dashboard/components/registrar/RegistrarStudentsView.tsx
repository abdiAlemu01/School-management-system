...
const streamBadge = (stream: string | null) => {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs border font-medium ${cfg}`}>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  <GraduationCap className={`w-3.5 h-3.5 shrink-0 ${active ? "text-indigo-400" : "text-slate-600"}`} />
  ...
  if (students.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-slate-400 font-medium text-sm">No students match your search</p>
        <p className="text-slate-600 text-xs mt-1">Try a different name or Student ID</p>
      </div>
    );
  }
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full text-sm">
        <thead className="bg-slate-950/40 border-b border-slate-800/60">
          <tr>
            <th className="text-left px-5 py-3 text-slate-400 font-semibold">Student</th>
            <th className="text-left px-5 py-3 text-slate-400 font-semibold">Student ID</th>
            <th className="text-left px-5 py-3 text-slate-400 font-semibold">Grade</th>
            <th className="text-left px-5 py-3 text-slate-400 font-semibold">Stream</th>
            <th className="text-left px-5 py-3 text-slate-400 font-semibold">Academic Year</th>
            <th className="text-left px-5 py-3 text-slate-400 font-semibold">Email</th>
          </tr>
        </thead>
        <tbody>
          {students.map((s) => (
            ...
            >
              <td className="px-5 py-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-xs capitalize">{s.gender}</p>
                  </div>
                </div>
              </td>
              <td className="px-5 py-3">
              ...
              <td className="px-5 py-3 text-slate-300 font-medium">Grade {s.grade}</td>
              <td className="px-5 py-3">{streamBadge(s.stream) ?? <span className="text-slate-600 text-xs">—</span>}</td>
              <td className="px-5 py-3 text-slate-400">{s.academicYear}</td>
              <td className="px-5 py-3">
  ...
  if (!yearGroups || yearGroups.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-32 bg-slate-900/50 rounded-2xl border border-slate-800/70">
        ...
        <p className="text-white font-medium">No students registered yet</p>
        <p className="text-slate-500 text-sm mt-1">
          Use the &quot;Register Student&quot; tab to add the first student.
        </p>
      </div>
    );
  }
      ...
      <aside className="w-56 xl:w-64 shrink-0 border-r border-slate-800/70 bg-slate-950/40 flex flex-col">
        ...
        <div className="px-4 py-4 border-b border-slate-800/60">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-semibold text-sm">Students</span>
          </div>
          <p className="text-slate-600 text-xs mt-0.5">{totalStudents} total enrolled</p>
        </div>
          ...
          {activeGroup && (
            <div className="relative">
              ...
              <input
                ...
                className="w-56 xl:w-72 bg-slate-950/60 border border-slate-800/70 text-white rounded-xl pl-9 pr-4 py-2 text-sm placeholder:text-slate-600 focus:outline-none focus:border-indigo-500/60 focus:ring-1 focus:ring-indigo-500/30 transition"
              />
            </div>
          )}
        </div>
        ...
        <div className="flex-1 overflow-auto">
          {!activeGroup ? (
            <div className="flex flex-col items-center justify-center h-full py-20 text-center">
              ...
              <p className="text-slate-500 text-sm">Pick a grade from the left panel</p>
            </div>
          ) : (
