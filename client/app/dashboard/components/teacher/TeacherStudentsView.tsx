  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-4">
        ...
        <span className="ml-auto px-2.5 py-0.5 rounded-full bg-slate-800/60 border border-slate-700/40 text-slate-400 text-sm font-semibold">
      ...
      <p className="text-slate-400 text-base mb-3">
        ...
        {sec.section && (
          <span className="ml-2 inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-indigo-600/20 border border-indigo-500/30 text-indigo-300 text-sm font-semibold">
      ...
      {/* Footer row */}
      <div className="flex items-center gap-3 text-sm text-slate-500 border-t border-slate-800/50 pt-3">
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <button
          onClick={onBack}
          className="flex items-center gap-1.5 text-slate-400 hover:text-white text-base transition mb-3"
        >
      ...
      {/* Info bar */}
      <div className="flex items-center gap-4 px-4 py-2.5 bg-slate-900/50 border border-slate-800/60 rounded-xl text-base flex-wrap">
        ...
        {!isLoading && (
          <div className="ml-auto flex items-center gap-1.5 px-3 py-1 bg-indigo-500/10 border border-indigo-500/20 rounded-lg">
            ...
            <span className="text-indigo-300 text-base font-semibold">{students.length} students</span>
          </div>
        )}
      </div>
      ...
      {!isLoading && !isError && students.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 bg-slate-900/50 rounded-2xl border border-slate-800/70 border-dashed">
          ...
          <p className="text-slate-500 text-base mt-1 max-w-xs text-center">
              ...
              <thead className="bg-slate-950/60 border-b border-slate-800/70">
                <tr>
                  {["#", "Student", "Student ID", "Email", "Section"].map((h) => (
                    <th key={h} className="text-left px-5 py-3 text-slate-400 font-semibold text-sm uppercase tracking-wider">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {students.map((s, idx) => (
                  <tr key={s.id} className="border-b border-slate-800/40 last:border-none hover:bg-slate-800/20 transition-colors">
                    <td className="px-5 py-3 text-slate-600 text-base">{idx + 1}</td>
                    ...
                    <td className="px-5 py-3">
                      <span className="inline-flex items-center gap-1.5 font-mono text-sm bg-slate-800/60 px-2.5 py-1 rounded-lg text-slate-300">
                    ...
                    <td className="px-5 py-3">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-sm font-semibold bg-indigo-500/15 text-indigo-300 border border-indigo-500/25">
                        <GraduationCap className="w-3 h-3" />
...
export default function TeacherStudentsView() {
  ...
  if (sections.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-slate-900/50 rounded-2xl border border-slate-800/70">
        ...
        <p className="text-white font-medium">No sections assigned for you yet.</p>
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-3xl font-bold text-white">My Students</h2>
        <p className="text-slate-400 text-base mt-0.5">
      ...
      {/* Summary bar */}
      <div className="flex items-center gap-3 px-4 py-2.5 bg-slate-900/50 border border-slate-800/60 rounded-xl text-base">
